...
public class CompilerTester {
    ...
    public static void main(String[] args) {
        ...
        // if (cmd.hasOption("int")) { // Interpreter mode - at this point the program
        // is well-formed
        // c.interpret(in);
        // } else {
                ...
                for (String cfg_output : cfg_output_options) {
                    switch (cfg_output) {
                        case "screen":
                            ...
                            break;
                        case "file":
                            String filename = sourceFile.substring(0, sourceFile.lastIndexOf('.')) + "_"
                                    + CFG_DOT_FILE_NAME;
                            try (PrintStream out = new PrintStream(GRAPH_DIR_NAME + File.pathSeparator + filename)) {
                                out.print(dotgraph_text);
                            } catch (IOException e) {
                                e.printStackTrace();
                                System.err.println("Error accessing the cfg file: " + GRAPH_DIR_NAME
        ...
        List<String> optArguments = (optArgs != null && optArgs.length != 0) ? Arrays.asList(optArgs)
        ...
        System.out.println("-".repeat(100));
        System.out.println(dotgraph_text);
        // we expect after this, there is file recording all transformations your
        // compiler did
        // e.g., if we run -s test000.txt -o cp -o cf -o dce -loop
        // the file will have the name "record_test000_cp_cf_dce_loop.txt"

        // Generate and print DLX Code
        int[] code = c.genCode();
        if (code.length > 0) {
            System.out.println("\nGenerated DLX Code:");
            System.out.println("-".repeat(100));
            for (int i = 0; i < code.length; i++) {
                System.out.print(DLX.instrString(code[i]));
            }
        }
    }
}
